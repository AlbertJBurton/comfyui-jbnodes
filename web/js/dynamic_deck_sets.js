// Dynamic Prompt Deck Set Handler for ComfyUI Custom Nodes
// --------------------------------------------------------
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData.name === "PromptLab") { 
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                const deckWidget = this.widgets.find((w) => w.name === "deck");
                const setWidget = this.widgets.find((w) => w.name === "set");

                if (deckWidget && setWidget) {
                    const originalCallback = deckWidget.callback;
                    
                    // Attach an event listener to the film_size dropdown
                    deckWidget.callback = async function (value, ...args) {
                        if (originalCallback) {
                            originalCallback.apply(this, [value, ...args]);
                        }
                        
                        // Prefer value from callback args (LiteGraph standard), fallback to widget value
                        const deckName = value || deckWidget.value;
                        if (!deckName) return;
                        
                        try {
                            // Query our custom Python API route
                            const response = await api.fetchApi(`/jbnodes/deck_sets?deck_name=${encodeURIComponent(deckName)}`);
                            const sets = await response.json();
                            
                            // Mutate the target widget's available options
                            setWidget.options.values = sets;
                            
                            // Check if the current value is still valid; if not, reset to the first available option
                            if (!sets.includes(setWidget.value)) {
                                setWidget.value = sets[0];
                            }
                            ...
                            console.error("[comfyui-jbnodes] Error fetching prompt deck sets:", err);
                        }
                    };
                    
                    // Trigger the callback once on instantiation to populate the initial state
                    setTimeout(() => {
                        if (deckWidget.callback) {
                            deckWidget.callback.call(deckWidget, deckWidget.value);
                        }
                    }, 100);
                }
            ...
            nodeType.prototype.onConfigure = function (info) {
                ...
                const deckWidget = this.widgets?.find((w) => w.name === "deck");
                if (deckWidget && deckWidget.callback) {
                    deckWidget.callback.call(deckWidget, deckWidget.value);
                }
